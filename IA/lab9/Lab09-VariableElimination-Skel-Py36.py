...
def _check_factor(_phi, all_vars, control):
    assert sorted(_phi.vars) == sorted(all_vars),         f"Wrong variables: {','.join(_phi.vars):s} instead of {','.join(all_vars):s}"
    assert len(_phi.values) == 2 ** len(all_vars),         f"Wrong number of entries in phi.values: {len(_phi.values):d}"
    n = len(all_vars)
    if n > 0:
        for j in range(n + 1):
            vals = [0] * (n - j) + [1] * j
