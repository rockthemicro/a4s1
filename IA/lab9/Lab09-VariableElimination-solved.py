...
def sum_out(var, phi):
    ...
    for vals, p in phi.values.items():
        key = vals[:ind] + vals[ind+1:]  # iau toate valorile pana la var si dupa var, adica pt (1, 0, 1) si var = B, iau (1, 1)
        if key in new_vals:  # o sa am mereu 2 probabilitati de adaugat pt o anumita keye (adica pt B = 0 si B = 1)
            new_vals[key] += p
        else:
            new_vals[key] = p
...
def prod_sum(var, Phi, verbose=False):
    ...
    # Cerinta 3:
    phi_x = list(filter(lambda x: var in x.vars, Phi))  # construiesc lista factorilor care-l contin pe vars
    
    if not [filter(lambda x: var not in x.vars, Phi)]:  # daca nu exista factori fara var, nu pot modifica factorii ?
        if verbose:
            print(Phi)
        return Phi
...
def variable_elimination(Phi, Z, verbose=False):
    # Cerinta 4:
    for Zi in Z:
        Phi = prod_sum(Zi, Phi, verbose)  # elimin toate variabilele din toti factorii
