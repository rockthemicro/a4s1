...
def Extend_graph(graph, all_actions):
    ...
    for (actiune, conditii) in all_actions.items(): # k este actiunea, v[0] este preconditia actiunii, v[1] este postconditia
        if included(conditii[0], state) and notAnyMutex(conditii[0], mutex):
            actions[actiune] = conditii
    
    # se calculează acțiunile mutual exclusive
    mutex_actions = []
    # TODO
    for (postconditie1, actiuni1) in actions.items():
        for (postconditie2, actiuni2) in actions.items():

            # nu ne intereseaza sa aplicam aceleasi actiuni, sau actiuni mutual exclusive
            if postconditie1 == postconditie2 or ((postconditie1, postconditie2) in mutex_actions or (postconditie2, postconditie1) in mutex_actions):
                continue


            # preconditii mutual exclusive (caz 1 actiuni mutual exclusive)
            comp_n = False
            for p1 in actiuni1[0]:
                for p2 in actiuni2[0]:
                    if isMutex(p1, p2, mutex):
                        comp_n = True
                        break
            if comp_n:
                mutex_actions.append((postconditie1, postconditie2))
                continue


            # efecte mutual exclusive (caz 2 actiuni mutual exclusive)
            inc_eff = False
            for e1 in actiuni1[1]:
                for e2 in actiuni2[1]:
                    if NOT(e1) == e2 or NOT(e2) == e1:
                        inc_eff = True
                        break
            if inc_eff:
                mutex_actions.append((postconditie1, postconditie2))
            ...
            # inferenta (caz 3 actiuni mutual exclusive)
            inter = False
            for p1 in actiuni1[0]:
                for e2 in actiuni2[1]:
                    if NOT(p1) == e2 and NOT(e2) == p1:
                        inter = True
                        break
            if inter:
                mutex_actions.append((postconditie1, postconditie2))
    ...
    # se calculează TOATE efectele acțiunilor de pe nivelul precedent
    # Stocați efectele ca un dicționar fapt (post conditie) -> listă acțiuni care produc efectul
    effects = {}
    # TODO
    for (actiune, conditii) in actions.items():
        for postconditii in conditii[1]:
            if postconditii in effects:
                effects[postconditii].append(actiune)
            else:
                effects[postconditii] = [actiune]

    # se calculează efectele mutual exclusive:
    # 2 efecte sunt mutex dacă nu pot rezulta din acțiuni care nu sunt mutual exclusive
    mutex_effects = []
    # TODO
    for (postconditie1, actiuni1) in effects.items():
        for (postconditie2, actiuni2) in effects.items():

            # nu ne intereseaza sa facem mutexuri intre efecte identice sau sa adaugam duplicate ale mutexurilor
            if postconditie1 == postconditie2 or ((postconditie1, postconditie2) in mutex_effects or (postconditie2, postconditie1) in mutex_effects):
                continue
            ...
            for a1 in actiuni1:
                ...
                if not notMutex:
                    break
            if notMutex:
                mutex_effects.append((postconditie1, postconditie2))
